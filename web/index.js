...
// ─── API Routes ──────────────────────────────────────────────────────────────
// Dev: inject session from env vars (no OAuth needed for testing)
// Prod: require valid Shopify session
const apiAuthMiddleware = isDev
  ? (req, res, next) => {
      res.locals.shopify = {
        session: {
...
app.use("/*", spaMiddleware, (req, res) => {
  const indexPath = path.resolve(STATIC_PATH, "index.html");
  res
    .status(200)
    .set("Content-Type", "text/html")
    .sendFile(indexPath, (err) => {
      if (err) {
        // Fallback inline HTML if dist not built yet
